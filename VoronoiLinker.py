...
def Do_Rotation_Converter(context, isS, isA, node_type):
    tree = context.space_data.edit_tree
    if not tree:
        return
...
class Pie_MT_Combine_Matrix(bpy.types.Menu):
    ...
    def draw(self, context):
        pie = self.layout.menu_pie()
        op = pie.operator('node.voronoi_mix_rotation', text='Combine Transform')
        op.node_type = 'FunctionNodeCombineTransform'
        op = pie.operator('node.voronoi_mix_rotation', text='Combine Matrix')
...
class Pie_MT_Separate_Matrix(bpy.types.Menu):
    ...
    def draw(self, context):
        pie = self.layout.menu_pie()
        op = pie.operator('node.voronoi_mix_rotation', text='Separate Matrix')
        op.node_type = 'FunctionNodeSeparateMatrix'
        op = pie.operator('node.voronoi_mix_rotation', text='Separate Transform')
...
class Pie_MT_Converter_Rotation_To(bpy.types.Menu):
    ...
    def draw(self, context):
        pie = self.layout.menu_pie()
        op = pie.operator('node.voronoi_mix_rotation', text='Rotation > Euler')
        op.node_type = 'FunctionNodeRotationToEuler'
        op = pie.operator('node.voronoi_mix_rotation', text='Rotation > Axis Angle')
...
class Pie_MT_Converter_To_Rotation(bpy.types.Menu):
    ...
    def draw(self, context):
        pie = self.layout.menu_pie()
        op = pie.operator('node.voronoi_mix_rotation', text='Euler > Rotation')
        op.node_type = 'FunctionNodeEulerToRotation'
        op = pie.operator('node.voronoi_mix_rotation', text='Axis Angle > Rotation')
...
class VoronoiQuickConstant(VoronoiToolTripleSk):
    ...
    def MatterPurposeTool(self, event, prefs, tree):
        ...
        # if not dict_qDM:
        #     return {'CANCELLED'}
        # isOutNdCol = skIn0.node.bl_idname==dict_qDM['RGBA'][0] #Заметка: Нод разделения; на выходе всегда флоаты.
        # isGeoTree = tree.bl_idname=='GeometryNodeTree'
        # isOutNdQuat = (isGeoTree)and(skIn0.node.bl_idname==dict_qDM['ROTATION'][0])
        # #Добавить:
        # bpy.ops.node.add_node('INVOKE_DEFAULT', type=dict_qDM[skIn0.type][isOutNdCol if not isOutNdQuat else 2], use_transform=not self.isPlaceImmediately)
        # aNd = tree.nodes.active
        # aNd.width = 140
        # if aNd.bl_idname in {dict_qDM['RGBA'][0], dict_qDM['VALUE'][1]}: #|3|.
        #     aNd.show_options = False #Как-то неэстетично прятать без разбору, поэтому проверка выше.
        # if skIn0.type in {'VECTOR', 'RGBA', 'ROTATION'}: #Зато экономия явных определений для каждого типа.
        #     aNd.inputs[0].hide_value = True
        # #Установить одинаковость режимов (например, RGB и HSV):
        # for li in GetListOfNdEnums(aNd):
        #     if hasattr(skIn0.node, li.identifier):
        #         setattr(aNd, li.identifier, getattr(skIn0.node, li.identifier))
        # #Соединить:
        # skIn = aNd.inputs[0]
        # for ski in aNd.inputs:
        #     if skIn0.name==ski.name:
        #         skIn = ski
        #         break
        if skIn0.type in ["ROTATION", "MATRIX"]:
            ...
            if hasattr(skIn0, "default_value"):
                ...
                if aNd.bl_idname == "FunctionNodeEulerToRotation":
                    aNd.inputs[0].default_value = value
                if aNd.bl_idname == "FunctionNodeInputBool":
                    aNd.boolean = value
                if aNd.bl_idname == "FunctionNodeInputString":
                    aNd.string = value
                if aNd.bl_idname == "GeometryNodeIndexSwitch":
                    aNd.data_type = "MENU"
                    aNd.show_options = False
                    id_name = skIn0.node.bl_idname
                    if id_name == "GeometryNodeMenuSwitch":
                        aNd.inputs[-1].hide = True
                        ...
                        for i in range(items_l):
                            aNd.inputs[i + 1].default_value = items[i].name
                    if id_name == "GeometryNodeIndexSwitch":
                        if skIn0.default_value != '':
                            aNd.inputs[1].default_value = skIn0.default_value
                if aNd.bl_idname == "ShaderNodeValue":
                    aNd.outputs[0].default_value = value
                if aNd.bl_idname == "FunctionNodeInputInt":
                    aNd.integer = value
                if aNd.bl_idname == "ShaderNodeCombineXYZ":
                    for i in range(3):
                        aNd.inputs[i].default_value = value[i]
                if aNd.bl_idname in ["FunctionNodeInputColor", "ShaderNodeRGB"]:
                    aNd.value = value
                    # aNd.color = value

SmartAddToRegAndAddToKmiDefs(VoronoiQuickConstant, "##A_C")
