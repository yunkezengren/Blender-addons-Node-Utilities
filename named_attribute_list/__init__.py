...
class ATTRLIST_OT_Add_Node_Change_Name_Type_Hide(Operator):
    ...
    def execute(self, context):
        ...
        if ui_type == 'ShaderNodeTree':
            ...
            attr_node = context.active_node
            if self.domain_str == "实例" or self.if_instanced:
    ...
    def draw(self, context):
        ...
        if scene.show_set_panel:
            ...
            if scene.add_settings:
                text = "非网格域存储属性节点名称接口连了别的字符串接口"
                box1.label(text="已知限制: " + text, icon='INFO')
                box1.label(text="               属性转到实例域后，在着色器添加属性，选项不一定会更改成实例")
...
class Cloud_Use_Translatation(Operator):
...
class NODE_OT_View_Stored_Attribute_Node_in_edit_tree(Operator):
    """在edit_tree里查找节点"""
...
def register():
    ...
    S.panel_info         = StringProperty(name='显示在面板上的描述', description='显示在n面板上的插件描述',  default="aaa")
