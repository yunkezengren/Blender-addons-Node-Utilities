...
class ModelBaseTool(BaseOperator, ProtocolTool):  #0
    ...
    def callback_draw_base(self, drawer: Drawer, context: Context):
        ...
        if self.prefs.draw_prefs.debug_field:
...
def unhide_node_reassign(nd: Node, self: ModelBaseTool, *, cond: bool, flag=None):
    # 折叠节点用合成接口位置, 不再强制展开+redraw. 展开会改布局、叠到别的节点上, 还容易读到过期的 runtime 坐标.
    return
