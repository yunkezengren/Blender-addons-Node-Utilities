...
def restore_collapsed_nodes(nodes):
    for nd in nodes:
        if dict_collapsedNodes.get(nd, None): # 工具在过程中可能会创建节点; 例如 vpt_rv_ee_is_save_preview_results.
            nd.hide = dict_collapsedNodes[nd]

def gen_tar_from_nd(nd: Node, pos: Vec2, ui_scale: float): # 从 nearest_nodes_tar 中提取出来, 本来没必要, 但 VLTT 逼我这么做.
    def distance_field(field0: Vec2, boxbou: Vec2): # 感谢 RayMarching, 没有它我不会想到这个.
    ...
    # 据我所知, 重路由节点与其他节点不同, 它的大小不会随着 ui_scale 的改变而改变. 所以它不需要除以 'ui_scale'.
    ndSize = Vec2((4, 4)) if isReroute else nd.dimensions/ui_scale
    ...
    # 构建距离场
    vec = distance_field(pos-ndCenter, ndSize)
    # 将处理过的节点添加到列表中
    return Target(nd, distance=vec.length, pos=pos-vec)

def nearest_nodes_tar(nodes, samplePos, ui_scale, includePoorNodes=True): # 返回最近的节点列表. 真实的距离场.
    ...
    for nd in nodes:
        ...
        tar_object = gen_tar_from_nd(nd, samplePos, ui_scale)
        valid_tars.append(tar_object)

    return sorted(valid_tars, key=lambda tar: tar.distance)

    # return sorted([gen_tar_from_nd(nd, samplePos, ui_scale) for nd in nodes if (nd.type!='FRAME')and( (nd.inputs)or(nd.outputs)or(includePoorNodes) )], key=lambda a:a.distance)
...
def gen_tars_from_puts(nd: Node, isSide, samplePos, ui_scale): # 为 vpt_rv_ee_sks_highlighting 提取出来.
    # 注意: 这个函数应该自己从标记中获取方向, 因为 `reversed(nd.inputs)`.
    def sk_is_linked_visible(sk: NodeSocket):
        if not sk.is_linked:
            return True
        return (sk.vl_sold_is_final_linked_cou)and(sk.vl_sold_links_final[0].is_muted)
    results: list[Target] = []
    ndDim = Vec2(nd.dimensions/ui_scale) # "nd.dimensions" 已经包含了界面缩放的校正, 所以把它返回到世界坐标系.
...
def nearest_sockets_tar(nd: Node, samplePos, ui_scale): # 返回"最近的插槽"列表. 真实的 Voronoi 图单元距离场. 没错, 这个插件就是因此得名的.
    if nd.type == 'REROUTE':
    ...
    tar_sks_in = gen_tars_from_puts(nd, False, samplePos, ui_scale)
    tar_sks_out = gen_tars_from_puts(nd, True, samplePos, ui_scale)
...
def is_classic_sk(sk: NodeSocket):
...
def select_and_active_nd_only(ndTar: Node):
...
def find_any_sk(nd: Node, tar_sks_in, tar_sks_out): # Todo0NA: 需要泛化!, 用 lambda. 并且外部循环遍历列表, 而不是两个循环.
    from ..node_items import NodeItemsUtils  # 延迟导入避免循环导入
    tar_sk_out, tar_sk_in = None, None
    for tar in tar_sks_out:
        if (tar.idname!='NodeSocketVirtual')and(NodeItemsUtils.is_item_socket_allowed(nd, tar.tar)): # todo1v6: 这个函数到处都和 !=NodeSocketVirtual 一起使用, 需要重做拓扑.
            tar_sk_out = tar
            break
...
def vlrt_remember_last_sockets(sko: NodeSocket, ski: NodeSocket):
    if sko:
...
def remember_add_link(sko: NodeSocket, ski: NodeSocket):
    ...
    vlrt_remember_last_sockets(sko, ski)

def do_quick_math(event, tree: NodeTree, operation, isCombo=False):
