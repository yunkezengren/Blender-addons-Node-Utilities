...
def GenFtgFromNd(nd: Node, pos: Vec2, uiScale: float): # 从 nearest_nodes_tar 中提取出来, 本来没必要, 但 VLTT 逼我这么做.
...
def nearest_nodes_tar(nodes, samplePos, uiScale, includePoorNodes=True): # 返回最近的节点列表. 真实的距离场.
    ...
    for nd in nodes:
        ...
        tar_object = GenFtgFromNd(nd, samplePos, uiScale)
...
def nearest_sockets_tar(nd: Node, samplePos, uiScale): # 返回"最近的插槽"列表. 真实的 Voronoi 图单元距离场. 没错, 这个插件就是因此得名的.
    ...
    tar_sks_in = GenFtgsFromPuts(nd, False, samplePos, uiScale)
    tar_sks_out = GenFtgsFromPuts(nd, True, samplePos, uiScale)
    tar_sks_in.sort(key=lambda a: a.dist)
    tar_sks_out.sort(key=lambda a: a.dist)
...
def MinFromFtgs(tar1: Target, tar2: Target):
    if (tar1)or(tar2): # 如果至少有一个存在.
        if not tar2: # 如果其中一个不存在,
            ...
            return tar1 if tar1.dist<tar2.dist else tar2
    return None
