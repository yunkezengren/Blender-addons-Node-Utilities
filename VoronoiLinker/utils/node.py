...
def GenTarFromNd(nd: Node, pos: Vec2, ui_scale: float): # 从 nearest_nodes_tar 中提取出来, 本来没必要, 但 VLTT 逼我这么做.
...
def nearest_nodes_tar(nodes, samplePos, ui_scale, includePoorNodes=True): # 返回最近的节点列表. 真实的距离场.
    ...
    for nd in nodes:
        ...
        tar_object = GenTarFromNd(nd, samplePos, ui_scale)
        valid_tars.append(tar_object)

    return sorted(valid_tars, key=lambda tar: tar.distance)

    # return sorted([GenTarFromNd(nd, samplePos, ui_scale) for nd in nodes if (nd.type!='FRAME')and( (nd.inputs)or(nd.outputs)or(includePoorNodes) )], key=lambda a:a.distance)
...
def GenTarsFromPuts(nd: Node, isSide, samplePos, ui_scale): # 为 vptRvEeSksHighlighting 提取出来.
...
def nearest_sockets_tar(nd: Node, samplePos, ui_scale): # 返回"最近的插槽"列表. 真实的 Voronoi 图单元距离场. 没错, 这个插件就是因此得名的.
    ...
    tar_sks_in = GenTarsFromPuts(nd, False, samplePos, ui_scale)
    tar_sks_out = GenTarsFromPuts(nd, True, samplePos, ui_scale)
