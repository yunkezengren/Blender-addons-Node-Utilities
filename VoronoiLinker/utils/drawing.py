...
class Drawer():
    ...
    def draw_line_strip(self, positions: Iterable[float2], colors: Iterable[float4], *, width: float) -> None:
        ...
        gpu_extras.batch.batch_for_shader(self.shaderLine, type='LINE_STRIP', content={'pos': positions, 'color': colors}).draw(self.shaderLine)
    ...
    def draw_rect(self, bounds1: float2, bounds2: float2, color: float4) -> None:
        self.draw_triangle_fan(((bounds1[0], bounds1[1]), (bounds2[0], bounds1[1]), (bounds2[0], bounds2[1]), (bounds1[0], bounds2[1])), color)
    ...
    def draw_circle(self, pos: float2, radius: float, *, resolution: int = 54, color: float4 = white_float4) -> None:
        ...
        self.draw_triangle_fan(((pos[0], pos[1]), *[(pos[0] + radius * cos(cyc * 2.0 * pi / resolution), pos[1] + radius * sin(cyc * 2.0 * pi / resolution))
                                                for cyc in range(resolution + 1)]), color)

    def draw_ring(self, pos: float2, radius: float, *, width: float, resolution: int = 16, color: float4 = white_float4, spin: float = 0.0) -> None:
        positions = tuple((radius * cos(cyc*2*pi/resolution + spin) + pos[0], radius * sin(cyc*2*pi/resolution + spin) + pos[1]) for cyc in range(resolution + 1))
        self.draw_line_strip(positions, (color, ) * (resolution+1), width=width)

    def draw_point_highlight(self, pos: float2, *, radius: float, color1: float4 = white_float4, color2: float4 = white_float4, resolution: int = 54) -> None:
...
def draw_socket_point(drawer: Drawer, pos: Vec2, *, color1: Color4 = white_float4, color2: Color4 = white_float4, resolution: int = 54, forcibly_color: bool = False) -> None: #"forcibly_color" 只用于 draw_debug_info.
    if not(drawer.dsIsColoredPoint or forcibly_color):
        color1 = color2 = drawer.dsUniformColor
    drawer.draw_point_highlight(drawer.ui_to_region(pos), radius=((6 * drawer.dsPointScale * drawer.worldZoom)**2 + 10)**0.5, color1=color1, color2=color2, resolution=resolution)
...
def draw_framed_text(drawer: Drawer, pos1: float2, pos2: float2, text: str, *, size: float, y_offset: float, text_color: float4, frame_color: float4, bg_color: float4) -> float2:
    pos1x = ps1x = pos1[0]
    pos1y = ps1y = pos1[1]
    pos2x = ps2x = pos2[0]
    pos2y = ps2y = pos2[1]
    ...
    match drawer.dsFrameDisplayType:
        case 2: # 漂亮的边框
            ...
            for cyc in range(gradResl):
                drawer.draw_rect( (pos1x, pos1y+cyc*gradStripHei),
                                     (pos2x, pos1y+cyc*gradStripHei+gradStripHei),
                                     (bg_color[0]/2, bg_color[1]/2, bg_color[2]/2, LFx(cyc/gradResl,0.2,0.05)*bg_color[3]) )
            # 明亮的主描边:
            drawer.draw_line_strip((pos1, (pos2x,pos1y), pos2, (pos1x,pos2y), pos1), (frame_color,)*5, width=1.0) # 天啊, 如果 frame_color[0]==-1, 结果会包含复数. 那里发生了什么?
            # 额外的柔和描边 (连同角落), 增加美感:
            ps1x += .25
            ps1y += .25
            ps2x -= .25
            ps2y -= .25
            ofs = 2.0
            positions = (  (ps1x, ps1y-ofs),  (ps2x, ps1y-ofs),  (ps2x+ofs, ps1y),  (ps2x+ofs, ps2y),
    ...
    blf.shadow(fontId, blur, 1.0, 1.0, 1.0, get_color_black_alpha(text_color, pw=3.0)*0.75)
...
def draw_world_text(drawer: Drawer, pos: Vec2, offset: float2, text: str, *, text_color: float4, bg_color: float4, font_size_override: float = 0) -> float2: # font_size_override 仅用于 vptRvEeSksHighlighting.
    ...
    return draw_framed_text(drawer, pos1, pos2, text, size=size, y_offset=dimDb[1]*drawer.dsManualAdjustment, text_color=text_color, frame_color=bg_color, bg_color=bg_color)   # 绘制颜色加深

def draw_socket_text(drawer: Drawer, pos: Vec2, offset: float2, target: Target, *, font_size_override: float = 0, tool_color: float4 = (0, 0, 0, 0)) -> float2: # 注意: `pos` 总是为了 drawer.cursorLoc, 但请参见 vptRvEeSksHighlighting.
    ...
    return draw_world_text(drawer, pos, offset, target.soldText, text_color=text_color, bg_color=bg_color, font_size_override=font_size_override)
...
def draw_node_template(drawer: Drawer, target_node: Target | None, *, side: int = 1, tool_name: str = "") -> None: # 模板重新思考过了; 很好. 现在它变得像其他所有的一样了.. 至少没有旧版本中的意大利面条式代码了.
    ...
    if target_node:
        ...
        if (drawer.dsIsDrawText)and(drawer.dsIsDrawNodeNameLabel):
            ...
            draw_world_text(drawer, drawer.cursorLoc, (drawer.dsDistFromCursor*side, -0.5), text, text_color=color_text, bg_color=color_text)
            draw_world_text(drawer, drawer.cursorLoc, (drawer.dsDistFromCursor*side, 1   ), _iface(tool_name), text_color=color_text, bg_color=color_text)
            # # 额外绘制
            # print(f"{text = }")
...
def draw_sockets_template(
    ...
    *args_targets: Target | None,
    ...
    for target in target_sockets:
        ...
        if drawer.dsIsDrawPoint:
            draw_socket_point(drawer, get_pos_from_target(target), color1=Color4(clamp_color4(get_sk_color(target.tar))), color2=Color4(get_sk_color_safe(target.tar)))
    # 文本
        ...
        for list_targets in targets_in, targets_out:  # "累积", 天才! 意大利面条式代码的头疼消失了.
            ...
            for cyc, target in enumerate(list_targets):
                ...
                if (drawer.dsIsDrawMarker) and ((target.tar.vl_sold_is_final_linked_cou) and (not is_draw_markers_more_than_one) or
                    ...
                    draw_socket_marker(drawer, cursor_loc, offset=(frame_dim[0] * dir, frame_dim[1] * ofs_y), color=get_sk_color_safe(target.tar))
            # 绘制工具提示
...
class TestDraw:
    ...
    def CallbackDrawTest(cls, context: Context):
        ...
        for cycWid in range(9):
            ...
            for cycAl in range(4):
                ...
                for cyc5 in range(2):
                    ofs5x = 65*cyc5
                    ofs5y = 0.5*cyc5
                    drawer.draw_line_strip(( (100+ofs5x,100+ofsWid+ofsAl+ofs5y),(165+ofs5x,100+ofsWid+ofsAl+ofs5y) ), (col, col), width=0.5*(1+cycWid))
        ##
        ...
        drawer.draw_ring((500,500), vec.length, width=cursorReg.x/200, resolution=max(3, int(cursorReg.y/20)), color=white_float4, spin=pi/2-atan2(vec.x, vec.y))
        # 混乱:
        center = Vec2((context.region.width/2, context.region.height/2))
        txt = "a.¯\_(- _-)_/¯"
        draw_framed_text(drawer, (300,300), (490,330), txt, size=24, y_offset=(555-525)*-.2, text_color=white_float4, frame_color=white_float4, bg_color=white_float4)
        txt = bpy.context.window_manager.clipboard
        txt = txt[0] if txt else "a."
        draw_framed_text(drawer, (375,170), (400,280), txt, size=24, y_offset=0, text_color=white_float4, frame_color=white_float4, bg_color=white_float4)
        draw_framed_text(drawer, (410,200), (435,250), txt, size=24, y_offset=0, text_color=white_float4, frame_color=white_float4, bg_color=white_float4)
        ...
        drawer.draw_line_strip( (center+Vec2((0,-60)), center+Vec2((100,-60))), (opaque_color4(col), opaque_color4(col)), width=drawer.dsLineWidth )
        drawer.draw_line_strip( (center+Vec2((100,-60)), center+Vec2((200,-60))), (opaque_color4(col), opaque_color4(col, alpha=0.0)), width=drawer.dsLineWidth )
        drawer.draw_point_highlight(center+Vec2((0,-60)), radius=( (6*drawer.dsPointScale+1)**2+10 )**0.5, color1=Color4(opaque_color4(col)), color2=Color4(opaque_color4(col)))
        drawer.draw_point_highlight(center+Vec2((100,-60)), radius=( (6*drawer.dsPointScale+1)**2+10 )**0.5, color1=col, color2=Color4(opaque_color4(col)))
