import bpy
from ..base_tool import SingleSocketTool
from ..common_class import VptData
from ..globals import voronoiAnchorCnName, voronoiAnchorDtName

class NODE_OT_voronoi_preview_anchor(SingleSocketTool):  # 嗯, 现在这是一个完整的工具了; 甚至可能需要在布局中为其创建一个新的独立类别.
    ...
    def find_targets(self, _is_first_active, prefs, tree):
        ...
        for tar_nd in self.get_nearest_nodes(cur_x_off=0):
            nd = tar_nd.tar
            tar_sks_out = self.get_nearest_sockets(nd, cur_x_off=0)[0]
    ...
    def run(self, event, prefs, tree):
        VptData.reprSkAnchor = repr(self.target_sk.tar)
    def initialize(self, event, prefs, tree):
        ...
        if self.anchorType:
            ...
            rrAnch.location = self.cursor_loc
