...
class NODE_OT_voronoi_linker(PairSocketTool):  # 神圣中的神圣. 它存在的理由. 最初的那个. 所有工具的老大. 为了伟大的距离场而荣耀!
    ...
    def find_targets_tool(self, isFirstActivation, prefs, tree): #Todo0NA ToolAssignmentFirst, Next, /^Root/; 多个 NA(), 第一个节点套接字, 第二个节点套接字.
        ...
        for ftgNd in self.get_nearest_nodes(cur_x_off=-20):
            nd = ftgNd.tar
            ...
            # 根据条件获取输入:
            sk_out = opt_ftg_socket(self.fotagoSkOut)
                    ...
                    if self.fotagoSkOut.tar.node==self.fotagoSkIn.tar.node: # 如果输出的最近输入是它自己的节点
                        ...
                        for lk in self.fotagoSkOut.tar.vl_sold_links_final:
                            if lk.to_socket==self.fotagoSkIn.tar: # 如果最近的输入是输出的连接之一, 则置空 => “期望的”连接已经存在.
                                ...
                                break
                    unhide_node_reassign(nd, self, cond=self.fotagoSkIn, flag=False) # “主流的”折叠处理.
            break # 只需要处理第一个最近的满足条件的. 否则结果会是最远的.
